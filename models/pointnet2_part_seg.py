...
def placeholder_inputs(batch_size, num_point, dimension):
    pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point, dimension))
    labels_pl = tf.placeholder(tf.int32, shape=(batch_size, num_point))
    return pointclouds_pl, labels_pl


def get_model_0 (point_cloud, is_training, bn_decay=None):
...
def get_model(point_cloud, is_training, bn_decay=None):
    """ Part segmentation PointNet, input is BxNxD (XYZ Feature), output Bx50 """
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value
    dim_point = point_cloud.get_shape()[2].value
    end_points = {}
    l0_xyz = tf.slice(point_cloud, [0,0,0], [-1,-1,3])
    l0_points = tf.slice(point_cloud, [0,0,3], [-1,-1,dim_point-3])
